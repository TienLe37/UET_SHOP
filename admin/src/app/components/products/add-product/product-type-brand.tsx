...
const ProductTypeBrand = ({
  ...
  if (!isLoading && !isError && brands?.success) {
    ...
    content = (
      <div className="mb-5">
        <p className="mb-0 text-base text-black">Thương hiệu</p>
        <Controller
          name="brand"
          control={control}
          rules={{
            required: default_value?.brand ? false : "Sản phẩm phải có thương hiệu!",
          }}
          render={({ field }) => (
            <ReactSelect
              {...field}
        ...
        <span className="text-tiny leading-4">Đặt thương hiệu cho sản phẩm.</span>
      </div>
    );
  }
  return (
    <div className="bg-white px-8 py-8 rounded-md mb-6">
      <div className="grid sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 2xl:grid-cols-2 gap-x-6">
        ...
        <div className="mb-5">
          ...
          <input
            id="unit"
            {...register("unit", { required: `Đơn vị sản phẩm là bắt buộc!` })}
          ...
          <span className="text-tiny leading-4">Đặt đơn vị của sản phẩm.</span>
        </div>
      </div>
    </div>
  );
