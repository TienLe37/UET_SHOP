...
const schema = Yup.object().shape({
  password: Yup.string().required("Vui lòng nhập mật khẩu hiện tại!").min(6, "Mật khẩu phải có ít nhất 6 kí tự").label("Password"),
  newPassword: Yup.string().required("Vui lòng nhập mật khẩu mới!").min(6, "Mật khẩu phải có ít nhất 6 kí tự").label("New Password"),
  ...
  const onSubmit = async (data: { password: string; newPassword: string }) => {
    if (user) {
      ...
      if ("error" in res) {
        ...
        notifySuccess("Đổi mật khấu thành công!");
        reset();
      }
    }
    reset();
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      <div className="mb-5">
        ...
        <input
          {...register("password", {
            required: "Vui lòng nhập mật khẩu hiện tại!",
          })}
          name="password"
