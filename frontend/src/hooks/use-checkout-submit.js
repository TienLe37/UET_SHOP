...
const useCheckoutSubmit = () => {
  ...
  const handleCouponCode = (e) => {
    ...
    if (!couponRef.current?.value) {
      notifyError("Vui lòng nhập coupon!");
      return;
    }
    if (isLoading) {
    ...
    if (isError) {
      return notifyError("Đã có lỗi xảy ra!");
    }
    ...
    if (result.length < 1) {
      notifyError("Vui lòng nhập coupon hợp lệ!");
      return;
    }
    ...
    if (total < result[0]?.minimumAmount) {
      notifyError(
        `Đơn hàng phải có giá trị tối thiểu ${formatPrice(result[0].minimumAmount)} để áp dụng coupon!`
      );
      return;
    } else {
      notifySuccess(
        `Mã giảm giá ${result[0].title} đã áp dụng lên ${result[0].productType}!`
      );
    ...
    try {
      ...
      .then((result) => {
          if(result?.error){
            ...
            notifySuccess("Đã đặt hàng thành công!");
          }
